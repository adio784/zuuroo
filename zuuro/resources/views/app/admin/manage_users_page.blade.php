                                  ...
                                  <div id="result">
                                    ...
                                    <tbody class="table-border-bottom-0">
                                      ...
                                      <tr>
                                        ...
                                        <td> <a class="dropdown-item" href="/view_user_info/{{ $user->id }}"> {{ $user->username }} </a></td>
                                        <td>
                                          <a href="tel:{{ $user->mobile }}" class="btn btn-sm btn-outline-primary p-1"> {{ $user->mobile }}</a>
                                        </td>
                                        ...
                                        <td>
                                          ...
                                          <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                              ...
                                              {{-- Suspend or make user an admin --}}
                                              {{-- @if($user->role == 1)
                                              <a class="dropdown-item" href="/remove_admin_page/{{ $user->id }}" onclick="return confirm('Are you sure to suspend this user from being an admin?')"
                                                ><i class="bx bx-trash me-2"></i> Remove Admin</a
                                              >@endif
                                              @if($user->role == 0)
                                                <a class="dropdown-item" href="/make_admin_page/{{ $user->id }}" onclick="return confirm('Are you sure to make this user an Admin?')"><i class="bx bx-user-pin me-2"></i> Make Admin</a>
                
                                              @endif --}}
                
                                              {{-- Change Admin Password --}}
                                              <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#modalCenter{{ $user->id }}" onclick="return confirm('Are you sure to changes this user password?')">
                                              ...
                                              <a class="dropdown-item" href="/verify_users_page/{{ $user->id }}" onclick="return confirm('Are you sure to verify this user?')"
                                                ><i class="bx bx-user-pin me-2"></i> Verify User</a
                                              >@endif
                
                                              {{-- Suspend or reactivate user --}}
                                              @if($user->status == 1)
                                              <a class="dropdown-item" href="/disable_users_page/{{ $user->id }}" onclick="return confirm('Are you sure to suspend this user?')"
                                      ...
                                      @include('app.admin.change_user_password')
                                      @endforeach
                                    </tbody>
                                </table>
                            </div>
